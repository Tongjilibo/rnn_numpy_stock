...
class RNN_NUMPY():
    ...
    def init_paras(self, X, y):
        ...
        self.paras['U'] = np.random.uniform(-np.sqrt(1. / self.input_dim), np.sqrt(1. / self.input_dim), (self.hidden_dim, self.input_dim))
        self.paras['V'] = np.random.uniform(-np.sqrt(1. / self.hidden_dim), np.sqrt(1. / self.hidden_dim), (self.output_dim, self.hidden_dim))
        self.paras['W'] = np.random.uniform(-np.sqrt(1. / self.hidden_dim), np.sqrt(1. / self.hidden_dim), (self.hidden_dim, self.hidden_dim))
...
if __name__ == '__main__':
    ...
    # 数据导入
    trainX, trainy, testX, testy, scaler = load_data('./data/stock_dataset_2.csv', 'high', n_in_timestep, n_out_timestep)

    # 建模
    model = RNN_NUMPY(hidden_dim=10, learning_rate=0.001, num_iter=100, batch_size=10)
